...
export function Router({ routes }: { routes: Routes }) {
  const [routerValue] = useState<RouterContextValue>({
    routes,
  });
...
function createSelectPathTool(paths: string[], currentPath: string) {
  const DecisionSchema = z
    ...
    .describe(
      `Examine the conversation history and select the next path to take. The possible paths are: ${paths.join(
        ", ",
      )}. If unsure select the current path.`,
    );
